...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fundo = fundos.find(f => f.cnpj === selecionado.cnpj)
    setFundoDetalhe(fundo || null)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: 'Inter, system-ui, sans-serif', background: '#f8fafc' }}>
      ...
      {/* Header */}
      <div style={{ background: 'white', borderBottom: '1px solid #e2e8f0', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap', zIndex: 1000 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '20px' }}>🗺️</span>
          <strong style={{ fontSize: '16px', color: '#0f172a' }}>FII Mapa</strong>
          <span style={{ fontSize: '12px', color: '#94a3b8', marginLeft: '4px' }}>Imóveis de Fundos Imobiliários</span>
        </div>
        ...
        {stats && (
          <div style={{ display: 'flex', gap: '16px', marginLeft: '8px' }}>
            {[
              { label: 'Imóveis', value: stats.com_coords.toLocaleString('pt-BR') },
              { label: 'Fundos', value: stats.total_fundos.toLocaleString('pt-BR') },
            ].map(s => (
              <div key={s.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '15px', fontWeight: 600, color: '#0f172a' }}>{s.value}</div>
        ...
        <div style={{ display: 'flex', gap: '8px', marginLeft: 'auto', flexWrap: 'wrap' }}>
          <select value={filtroFundo} onChange={e => setFiltroFundo(e.target.value)}
            style={{ padding: '6px 10px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '13px', color: '#334155', background: 'white', minWidth: '200px' }}>
        ...
        <span style={{ fontSize: '12px', color: '#94a3b8' }}>{imoveis.length} no mapa</span>
      </div>

      {/* Legenda de tipos */}
      <div style={{ background: 'white', borderBottom: '1px solid #e2e8f0', padding: '6px 20px', display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
        {Object.entries(TIPO_CORES).map(([tipo, cor]) => (
          <div key={tipo} style={{ display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer' }}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: "hidden", height: "100%" }}>
        ...
        {/* Mapa */}
        <MapContainer center={[-15.7801, -47.9292]} zoom={4} style={{ flex: 1, height: "100%" }} zoomControl={true}>
          <TileLayer
            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'
            url='https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png'
          />
          <ClusterLayer imoveis={imoveis} onSelect={setSelecionado} />
        </MapContainer>

        {/* Painel lateral */}
        {selecionado && (
          <div style={{
            width: '340px', background: 'white', borderLeft: '1px solid #e2e8f0',
            ...
            <div style={{ padding: '16px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ fontSize: '14px', fontWeight: 600, color: '#0f172a', marginBottom: '4px' }}>{selecionado.descricao}</div>
              ...
              <button onClick={() => setSelecionado(null)}
                style={{ background: 'none', border: 'none', fontSize: '18px', cursor: 'pointer', color: '#94a3b8', padding: '0 0 0 8px' }}>×</button>
            </div>
            ...
            <div style={{ padding: '16px', borderBottom: '1px solid #e2e8f0' }}>
              ...
              {[
                { icon: '📍', label: selecionado.endereco },
                { icon: '📐', label: selecionado.area_m2 ? `${Number(selecionado.area_m2).toLocaleString('pt-BR')} m²` : '—' },
              ].map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '8px', fontSize: '13px', color: '#334155' }}>
                  <span>{item.icon}</span>
            ...
            {imoveisFundo.length > 1 && (
              <div style={{ padding: '16px' }}>
                ...
                {imoveisFundo
                  .filter(im => im.id !== selecionado.id)
                  .map(im => (
                    <div key={im.id}
                      onClick={() => setSelecionado(im)}
                      style={{ padding: '8px', borderRadius: '8px', marginBottom: '6px', cursor: 'pointer',
